...
based on the circumstances that makes physicians successfull.
...
Another interesting thing to note here is that sex doesn't seem to be a major factor in deciding the outcome and for risk scores of more than 3.5 the chances of a successfull outcome is only 31%.
...
These factors could be symptoms, diet, allergies, health history, etc. Even A few other factors that come to mind are
- Diet : This could helps find a correlation between food and the disease and provide dietary advice to the patients.
- Geography/Location:  This could relate to diet and also help in recommmending physicians who are close to the patients.
- Symptoms, Allergies, Health history : This could help us with early detection of the disease
- Sex: We know that the member's sex is part of the risk score. But building the tree without the risk score and using sex as a feature, we could see a split on sex in the decisison tree.
- Treatment: What treatments were suggested? Which prove to be most successful and for what kind of patients? We can also share this information with the physicians.
...
- When the physician makes an order for prescriptions, he/she has a record of the quantity ordered in a database.
- The physician enters the prescription given to each of their patients in a measurable format like dosage per day. Also, if the prescriptions are fullfilled through the physician or through a pharmacy and what quantity was given out during the patient's first visit (or first diagnosis).

Given this information, we can predict on which date a physician will run out of a prescription. This can be continually updated based as the physicians order new stock of prescriptions or hand out prescirptions to new patients or update for current patients.
We can add a pop-up/email reminder 7 days before we think the physician will run out the prescriptions. To attribute the ordering of the physician to the alert, we can assume that if the physician orders the prescription within 2 days of acknowleding the alert.

We can divide our set of physicians into a test and control group and use the metric of quantity left before placing order.

Some of the challenges include:

- making sure not to include physicians who are generally good at keeping up the quantity. Randomness in testing.
- hard to be sure that physician is ordering because of seeing the alert. Maybe adding the order button on the alert will help.
- have enough of a sample size to make a statisitical significance in effect of of the feature.
- how long to run ?
- Is it risky to play with health ?
- Quantity of prescriptions is different for each type.
