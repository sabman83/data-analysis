...
In order to define who the best and the worst users are, I collect information about the number of pings, number of days logged in and the average pings per day for each user.
...
While those at the extreme left are our worst users. Coloring the users by their sources don't give much information as the sources seem to be widely distributed between the range.
...
But if the user has only used the app for, say, 10 days and hasn't used since , then we have probabaly lost the interest of the user.

There are possibly many ways to combine the number of days and pings to get a measure of active users. For the purposes of this assignment, I will calculate the visit rate for each of the user
which is the number of days they have been active since joining during the duration of February 2016. Those users who do not have a *used_first_time_today*, I assume that they have joined before
...
extreme ends of the distribution. Some of the users with 100 % or a very low visit rate might be users who joined late in Februray and we don't have
...
Before analyzing the data for identifying the sources which bring in the most favorable users, I remove users who joined the app recently since we do not have
...
For the prupose of this analysis, I only consider sources with number of users more than 25.
...
sample sizes we would need to perform statistical analysis to find out if the difference in means between these groups are significant enough to warrant
...
>
~~~
From the above pairwise comparisons the sources with high average visit rate like facebook and  liveintent show a significant difference compared to brand1, bing and program.
...
to sources like Facebook and Liveintent.


### Notes

* Some of the sources could be considered the one and the same like : bing and Bing, brand1 and [brand1,+brand1], facebook and Facebook_org and facebook.com, etc. I considered these sources to be separate since that might be the business intent. In some cases, the sample size were so small (Bing only had 4 users) that
it was safe to ignore them. Given more time, I would have checked with the Product team if these sources should indeed be considered as one and the same.
...
* I noticed that for one of the users had more than one date as the first usage date.
...
~~~
When filtering the data I used the earliest date. Since it was only for one user I don't think it affects our analysis.


## Recommendations for Grammarly

* I noticed that Grammarly provides vocabulary suggestions. I think it would be also useful to help users build their vocabulary and learn new words.
...
at the click of a button. When the user uses a word that has an interesting history associated with its origin or looks up the meaning of a word then adding a
...
Also, when the user makes an error, then showing a statistic like how many other users make the same mistake (or maybe how the percentile of the users)
would make for an interesting user experience.

* If we can detect the typing speed of the users and if we can establish a relationship between the numbers of errors they make with the typing speed, then we cane make a suggestion
to either lower or increase the typing speed.

* We can analyze the written data to see if certain words shows an increase in usage at a certain point in time and if so identify what caused it like politcal events, pop culture, etc.
...
* Instead of a fixed pricing charge, maybe Grammarly can consider an usage based charge. Based on the usage pattern of users, we can help come up with a pricing point that might attract users who have left or are planning to leave.
