...
View(duplicate_rows)
grly_table <- grly_table %>% group_by(attributed_to, date_time) %>% filter(!(n() > 1 & is.na(first_utm_source))) %>% ungroup()
...
> nrow(grly_table %>% group_by(attributed_to) %>% filter(used_first_time_today == TRUE) %>% filter(length(unique(date)) >1))
[1] 2
...
new_users_of_4th <- grly_table %>% filter(date == fourth & used_first_time_today == TRUE)  %>% select(attributed_to)
new_users_of_10th <- grly_table %>% filter(date == tenth & used_first_time_today == TRUE)  %>% select(attributed_to)
new_users_of_4th <- unique(new_users_of_4th$attributed_to)
new_users_of_10th <- unique(new_users_of_10th$attributed_to)

retention_data <- grly_table %>% group_by(date) %>% summarise(retention_rate_for_4th = length(intersect(new_users_of_4th,unique(attributed_to))) / length(new_users_of_4th), retention_rate_for_10th = length(intersect(new_users_of_10th,unique(attributed_to))) / length(new_users_of_10th))
melted_retention_data <- melt(retention_data, id="date")
ggplot(data = melted_retention_data, aes(x=date,y= value * 100, colour = variable))  +geom_line() + labs(x = "Date (2016)", y = "Retention in %")+ ggtitle("Daily Retention Curve for Cohorts of February 4th and 10th, 2016") + theme(plot.title = element_text(face = "bold")) + scale_colour_discrete(name="Cohorts", labels = c("4th February 2016", "10th February 2016")) + scale_y_discrete(limits = seq(from=0, to=100, by = 10))
